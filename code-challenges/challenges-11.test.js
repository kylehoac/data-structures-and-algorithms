...
const count = (target, input) => (input.flat().reduce((acc, cur) => cur === target ? acc + 1 : acc, 0));
...
const totalSum = (input) => input.flat().reduce((acc, cur) => acc + cur, 0);
...
let findMaleAndFemale = (data) => ( data.filter(human => human.gender === 'male' || human.gender === 'female').map(i => i.name).join(" and ")
);
