...
const longestString = (arr) => {
  let str = arr.reduce((acc,cur)=> acc.length < cur.length ? acc = cur : acc, '');
...
const onlyOddChars = (str) => str.split('').filter((val,idx) => idx % 2 !== 0).join('');
...
const allHappy = (arr) => arr.reduce((acc,cur) => cur.includes(':)') ? acc : acc = false, true );
